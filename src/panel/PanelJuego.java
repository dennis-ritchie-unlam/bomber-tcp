...
public class PanelJuego extends JPanel {
	...
	public PanelJuego() {
		...
		for (int k = 0; k < 4; k++)
			for (int i = 0; i < 4; i++)
				for (int j = 0; j < 5; j++)
					matrizBomberIcons[k][i][j] = new ImageIcon(
							"./Images/Bombermans/Player " + (k + 1) + "/" + i + "" + (j + 1) + ".gif");
		...
		for (Iterator<Bomba> iterator = bombas.iterator(); iterator.hasNext();) {
			...
			if (bomba.isExploto()) {
				...
				if (!mapa.hayAlgo(bomba.getPosicionX() / BLOCK_SIZE + 1, bomba.getPosicionY() / BLOCK_SIZE))
					g.drawImage(fuegoIcon.getImage(), bomba.getPosicionX() + BLOCK_SIZE, bomba.getPosicionY(),
							BLOCK_SIZE, BLOCK_SIZE, null);
				if (!mapa.hayAlgo(bomba.getPosicionX() / BLOCK_SIZE - 1, bomba.getPosicionY() / BLOCK_SIZE))
					g.drawImage(fuegoIcon.getImage(), bomba.getPosicionX() - BLOCK_SIZE, bomba.getPosicionY(),
							BLOCK_SIZE, BLOCK_SIZE, null);
				if (!mapa.hayAlgo(bomba.getPosicionX() / BLOCK_SIZE, bomba.getPosicionY() / BLOCK_SIZE + 1))
					g.drawImage(fuegoIcon.getImage(), bomba.getPosicionX(), bomba.getPosicionY() + BLOCK_SIZE,
							BLOCK_SIZE, BLOCK_SIZE, null);
				if (!mapa.hayAlgo(bomba.getPosicionX() / BLOCK_SIZE, bomba.getPosicionY() / BLOCK_SIZE - 1))
					g.drawImage(fuegoIcon.getImage(), bomba.getPosicionX(), bomba.getPosicionY() - BLOCK_SIZE,
							BLOCK_SIZE, BLOCK_SIZE, null);
